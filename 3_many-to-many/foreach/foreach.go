//go:generate scribblec-param.sh ../ManyToMany.scr -d ../ -param Foreach github.com/nickng/scribble-go-examples/3_many-to-many/ManyToMany -param-api A -param-api B
...
import (
	...
	"github.com/nickng/scribble-go-examples/3_many-to-many/ManyToMany/Foreach/A_1toM"
	"github.com/nickng/scribble-go-examples/3_many-to-many/ManyToMany/Foreach/B_1toN"
...
func A(p *Foreach.Foreach, M, N, selfM int, cc scributil.ClientConn, host string, port int, wg *sync.WaitGroup) {
	...
	A.Run(func(s *A_1toM.Init) A_1toM.End {
		...
		sEnd := s.Foreach(func(s *A_1toM.Init_6) A_1toM.End {
			sEnd := s.B_J_Scatter_Data(d)
			return *sEnd
		})
...
func B(p *Foreach.Foreach, M, N, selfN int, sc scributil.ServerConn, port int, wg *sync.WaitGroup) {
